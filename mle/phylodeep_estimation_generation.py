...
if TARGET_TIP_SIZES is None:
    ...
    print(
        f"Tip sizes to process: {unique_sizes[:10]}{'...' if len(unique_sizes) > 10 else ''} ({len(unique_sizes)} unique sizes)"
    )
            ...
            for model, model_name in zip(available_models, model_names):
                try:
                    ...
                    result = {
                        ...
                        "model_selection_prob": best_prob
...
for tip_size in TARGET_TIP_SIZES:
    ...
    print(f"\nTip Size: n={tip_size}")
    print(
        f"Total trees analyzed: {len(df_size[df_size['is_best_model'] == True].drop_duplicates('tree_idx'))}"
    )
